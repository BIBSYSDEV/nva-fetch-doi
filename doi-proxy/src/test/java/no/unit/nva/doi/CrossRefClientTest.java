...
public class CrossRefClientTest {
  ...
  @Test
  void createTargetUrlReturnsAValidUrlForDoiStringThatIsNotDoiURL() throws JsonProcessingException {
  ...
  void fetchDataForDoiReturnAnEmptyOptionalForANonExistingUrl() throws JsonProcessingException {
    ...
    var result =
        crossRefClient.fetchDataForDoi(DOI_STRING).map(MetadataAndContentLocation::getJson);
  ...
  void crossrefClientThrowsRuntimeExceptionIfEnvironmentVariableCrossrefApiTokenNameIsMissing() {
    Executable executable =
        () -> getConfiguredCrossrefClient(mock(HttpClient.class), false, true, true);
  ...
  void crossrefClientThrowsRuntimeExceptionIfEnvironmentVariableCrossrefApiTokenKeyIsMissing() {
    Executable executable =
        () -> getConfiguredCrossrefClient(mock(HttpClient.class), true, false, true);
    var exception = assertThrows(RuntimeException.class, executable);
    var actual = exception.getMessage();
  ...
  void crossrefClientLogsErrorIfEnvironmentVariableCrossrefApiTokensNameIsMissing() {
    var log = LogRecorder.forRoot(CrossRefClient.class);
    Executable executable =
        () -> getConfiguredCrossrefClient(mock(HttpClient.class), false, false, true);
    assertThrows(RuntimeException.class, executable);
    assertThat(log.messages(), hasItem(containsString(CROSSREFPLUSAPITOKEN_NAME_ENV)));
  }
  ...
  void crossrefClientThrowsRuntimeExceptionWhenSecretsManagerLacksSecret() {
    Executable executable =
        () ->
            getConfiguredCrossrefClient(mock(HttpClient.class), true, true, false)
                .fetchDataForDoi(DOI_STRING);
    RuntimeException exception = assertThrows(RuntimeException.class, executable);
  ...
  @Test
  void fetchDataForDoiReturnsNotFoundWhenInputDoiDoesNotDereference()
      throws URISyntaxException, JsonProcessingException {
  ...
  private CrossRefClient getConfiguredCrossrefClient(
      HttpClient httpClient,
      boolean withApiTokenName,
      boolean withApiTokenKey,
      boolean withApiSecret)
      throws JsonProcessingException {
    ...
    if (withApiSecret) {
      ...
      var secretValue =
          GetSecretValueResponse.builder().name(NAME).secretString(secretString).build();
  ...
  private CrossRefClient crossRefClientReceives404() throws JsonProcessingException {
    var errorResponse = new HttpResponseStatus404<>(ERROR_MESSAGE);
  ...
  private CrossRefClient crossRefClientReceives500() throws JsonProcessingException {
    var errorResponse = new HttpResponseStatus500<>(ERROR_MESSAGE);
    var mockHttpClient = new MockHttpClient<>(errorResponse);
    return getConfiguredCrossrefClient(mockHttpClient);
  }
}
