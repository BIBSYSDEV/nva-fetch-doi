...
public class DoiProxyService {
  ...
  public DoiProxyService(Environment environment) {
    this(
        new CrossRefClient(HttpClient.newHttpClient(), environment, new SecretsReader()),
        new DataciteClient());
  }
   ...
   */
  public MetadataAndContentLocation lookupDoiMetadata(
      String doiUrl, DataciteContentType dataciteContentType)
    ...
    LOGGER.info(
        "Received response from Crossref after {} ms",
        (crossRefEndTime - crossRefStartTime) / 1000);
    if (crossRefResult.isEmpty()) {
      ...
      LOGGER.info(
          "Received response from Datacite after {} ms",
          (dataciteEndTime - dataciteStartTime) / 1000);
    } else {
