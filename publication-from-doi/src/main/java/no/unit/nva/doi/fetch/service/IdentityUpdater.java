...
public final class IdentityUpdater {
  ...
  public static final String PROBLEM_UPDATING_IDENTITY_MESSAGE =
      "Problem updating Identity, ignoring and moving on";
   ...
   * @return an updated publication with identifiers added to identities or the original if
   *     unchanged or exception occurs for some reason
   */
  public static CreatePublicationRequest enrichPublicationCreators(
      CristinClient cristinClient, CreatePublicationRequest publication) {

    var possibleContributors = extractPossibleContributors(publication);
    possibleContributors.ifPresent(
        contributors ->
            tryUpdatingContributorsOrLogError(cristinClient, publication, contributors));
    return publication;
  }

  private static Optional<List<Contributor>> extractPossibleContributors(
  ...
  private static void tryUpdatingContributorsOrLogError(
  ...
  private static void updateContributors(
      CristinClient cristinClient,
      CreatePublicationRequest publication,
      List<Contributor> contributors) {
    var contributorsWithOnlyOrcid =
        contributors.stream().filter(IdentityUpdater::hasOrcidButNotIdentifier).toList();
    if (contributorsWithOnlyOrcid.size() > MAX_CONTRIBUTORS_TO_LOOKUP) {
      LOGGER.warn(
          "Skipping updateContributors as too many without known cristin-identifier: {}",
          publication.getEntityDescription().getMetadataSource());
      return;
    }

    contributorsWithOnlyOrcid.forEach(
        contributor -> updateIdentifierIfFoundFromOrcid(cristinClient, contributor.identity()));
  }
  ...
  private static void updateIdentifierIfFoundFromOrcid(
      CristinClient cristinClient, Identity identity) {
    ...
    if (person.isPresent()) {
      ...
      identity.setVerificationStatus(
          person
              .map(CristinPersonDto::verified)
              .map(VerificationStatus::fromBoolean)
              .orElse(NOT_VERIFIED));
    }
  }
}
