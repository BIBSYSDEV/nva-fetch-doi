...
public class ImportDoiHandler extends ApiGatewayHandler<RequestBody, Summary> {
  ...
  public ImportDoiHandler(Environment environment) {
    this(
        ...
        new DoiTransformService(),
        new DoiProxyService(environment),
        new PublicationPersistenceService(),
  ...
  public ImportDoiHandler(
    ...
    this.fetchDoiService =
        new FetchDoiService(doiTransformService, doiProxyService, cristinClient, metadataService);
  }
    ...
    return attempt(() -> this.fetchDoiService.newCreatePublicationRequest(inputUri))
        .map(
            createPublicationRequest ->
                tryCreatePublication(authHeader, apiUrl, createPublicationRequest))
  ...
  private PublicationResponse tryCreatePublication(
      String authorization, URI apiUrl, CreatePublicationRequest request)
      throws InterruptedException, IOException, CreatePublicationException {
    return createPublication(authorization, apiUrl, request);
  }

  private PublicationResponse createPublication(
      String authorization, URI apiUrl, CreatePublicationRequest request)
