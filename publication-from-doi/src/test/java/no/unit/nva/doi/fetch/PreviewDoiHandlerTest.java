...
class PreviewDoiHandlerTest extends DoiHandlerTestUtils {
  ...
  @Test
  void shouldReturnCreatePublicationRequestGivenValidInput() throws Exception {
  ...
  void shouldReturnBadGatewayWhenDoiProxyThrows() throws Exception {
    ...
    assertThat(
        getProblemDetail(gatewayResponse), containsString("Failed to fetch metadata from URL"));
  }
  ...
  GatewayResponse<CreatePublicationRequest> parseSuccessResponse(String output)
  ...
  PreviewDoiHandler createHandler(Environment environment)
    ...
    return new PreviewDoiHandler(
        doiTransformService, doiProxyService, cristinClient, metadataService, environment);
  }
