...
public class CrossrefDocumentTest {
  ...
  private static final Path CROSSREF_BOOK_CHAPTER_RESOURCE =
      Path.of("crossref_sample_book_chapter.json");
  private static final Path CROSSREF_EDITED_BOOK_RESOURCE =
      Path.of("crossref_sample_edited_book.json");

  @Test
  public void testSettersAndGetters() throws IOException {

    String resourceString = IoUtils.stringFromResources(CROSSREF_RESOURCE);
    CrossrefApiResponse crossRefResponse =
        Json.readValue(resourceString, CrossrefApiResponse.class);
    String serializedObject = Json.writeValueAsString(crossRefResponse);
    CrossrefApiResponse deserializedObject =
        Json.readValue(serializedObject, CrossrefApiResponse.class);

    assertThat(
        Json.convertValue(deserializedObject, JsonNode.class),
