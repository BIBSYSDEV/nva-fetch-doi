...
class DataciteTypesUtilTest {
  ...
  @DisplayName(
      "The mapper util checks for resourceTypeGeneral and returns null for non-text values")
  @Test
  ...
  @DisplayName(
      "The mapping utility returns type JOURNAL_CONTENT when all input types map to"
          + " JOURNAL_CONTENT")
  @Test
  void mapToTypeReturnsJournalContentWhenAllTypesAreJournalContent() {
    DataciteTypes dataciteTypes =
        new DataciteTypes.Builder()
    ...
    DataciteResponse dataciteResponse =
        new DataciteResponse.Builder().withTypes(dataciteTypes).build();
    assertEquals(
        PublicationType.JOURNAL_CONTENT,
        DataciteTypesUtil.mapToType(dataciteResponse).orElseThrow());
  }

  @DisplayName(
      "The mapping utility returns type JOURNAL_CONTENT when all but one types map to"
          + " JOURNAL_CONTENT")
  @Test
  ...
  @DisplayName(
      "The mapping utility returns type JOURNAL_CONTENT when all but two types map to"
          + " JOURNAL_CONTENT")
  @Test
  ...
  @DisplayName(
      "The mapping utility returns type JOURNAL_CONTENT when two types map to JOURNAL_CONTENT")
  @Test
  void mapToTypeReturnsJournalContentWhenTwoTypesAreJournalContent() {
    var dataciteTypes =
        new DataciteTypes.Builder()
            ...
            .withResourceTypeGeneral("Text")
            .build();
    var dataciteResponse = new DataciteResponse.Builder().withTypes(dataciteTypes).build();
    assertEquals(
        PublicationType.JOURNAL_CONTENT,
        DataciteTypesUtil.mapToType(dataciteResponse).orElseThrow());
  }
  ...
  private DataciteResponse getNonTextDataciteResource() {
    DataciteTypes dataciteType = new DataciteTypes.Builder().withResourceType(DATASET).build();
    return new DataciteResponse.Builder().withTypes(dataciteType).build();
  }
}
