...
public class ExtractionPair {
  private static final Set<IRI> CONTRIBUTOR_PROPERTIES =
      Set.of(DcTerms.CREATOR.getIri(), DcTerms.CONTRIBUTOR.getIri());
  public static final Set<IRI> DATE_PREDICATES =
      Set.of(
          DcTerms.DATE.getIri(),
          DcTerms.DATE_ACCEPTED.getIri(),
          DcTerms.DATE_COPYRIGHTED.getIri(),
          DcTerms.DATE_SUBMITTED.getIri());
  public static final String DATE_REGEX = "^[0-9]{4}(-[0-9]{2}(-[0-9]{2})?)?$";
  public static final Set<IRI> TAG_IRIS =
      Set.of(
          DcTerms.COVERAGE.getIri(),
          DcTerms.TEMPORAL.getIri(),
          DcTerms.SPATIAL.getIri(),
          DcTerms.SUBJECT.getIri());
  private final Statement statement;
  private final EntityDescription entityDescription;
  private final boolean abstractPropertyExists;

  public ExtractionPair(
      Statement statement, EntityDescription entityDescription, boolean abstractPropertyExists) {
  ...
  public boolean isDoi() {
    return Bibo.DOI.getIri().equals(statement.getPredicate())
