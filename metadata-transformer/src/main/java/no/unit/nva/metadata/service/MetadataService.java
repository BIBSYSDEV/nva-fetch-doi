...
public class MetadataService {
  ...
  private static final String DOI_DISPLAY_REGEX =
      "(doi:|doc:|http(s)?://(dx\\.)?doi\\.org/)?10\\.\\d{4,9}+/.*";
  ...
  public Optional<CreatePublicationRequest> generateCreatePublicationRequest(URI uri)
    ...
    try {
      ...
      throw new MetadataFetchException(
          "Failed to fetch metadata from: %s".formatted(uri), exception);
    }
  }

  public Optional<URI> fetchJournalIdFromPublicationChannel(
      String name, String electronicIssn, String printedIssn, int year) {
    return Stream.of(electronicIssn, printedIssn, name)
  ...
  private Model getMetadata(URI uri)
      throws ExtractionException, IOException, URISyntaxException, InterruptedException {
    ...
    try (RepositoryConnection repositoryConnection = db.getConnection()) {
      ...
      try (RepositoryResult<Statement> statements =
          repositoryConnection.getStatements(null, null, null)) {
  ...
  private void extractKnownProperties(Model model, Statement statement)
      throws IOException, InterruptedException {
  ...
  private Value extractValue(OntologyProperty ontologyProperty, Value object)
      throws IOException, InterruptedException {
  ...
  private Optional<String> fetchDoiUriFromShortDoi(String value)
      throws IOException, InterruptedException {
    ...
    return response.statusCode() == MOVED_PERMANENTLY
  ...
  private Optional<URI> fetchPublicationIdentifierFromPublicationChannels(
  ...
  private Optional<URI> queryPublicationChannelForJournal(String term, int year) {
  ...
  private Optional<URI> queryPublicationChannelForPublisher(String term) {
  ...
  private Optional<URI> parseResponseFromPublicationChannelsProxy(HttpResponse<String> response) {
  ...
  private Optional<URI> getIdOfFirstElement(HttpResponse<String> response) {
  ...
  private HttpResponse<String> sendQueryToPublicationChannelsProxy(String term, int year)
    ...
    var uri =
        UriWrapper.fromUri(publicationChannelsHostUri)
    ...
    var request =
        HttpRequest.newBuilder()
            .uri(uri)
            .headers(
                HttpHeaders.ACCESS_CONTROL_ALLOW_HEADERS, MediaTypes.APPLICATION_JSON_LD.toString())
            .GET()
            .build();
  ...
  private HttpResponse<String> sendQueryToPublicationChannelsProxy(String term)
    ...
    var searchTerm = URLEncoder.encode(term, StandardCharsets.UTF_8);
    var uri =
        UriWrapper.fromUri(publicationChannelsHostUriPublisher)
            .addQueryParameter(QUERY_PARAM_QUERY, searchTerm)
            .getUri();
  ...
  /**
   * Fetches publisherId as URI from publication-channels by publisherName only with exact single
   * match or returns null.
   *
   * @param publisherName the name of the publisher
   * @return jsonString representing the publisher
   */
  public URI fetchPublisherIdFromPublicationChannel(String publisherName) {
    return Optional.ofNullable(publisherName)
        .flatMap(this::queryPublicationChannelForPublisher)
        .stream()
